...
| `class_types` | `name` (coletiva, turma kids, personal, avaliação…) | Afeta valor, se configurado |
...
| `schedule_slot_versions` | `slot_id`, `weekday (1=seg…7=dom)`, `start_time`, `duration_min`, `modality_id`, `class_type_id?`, `valid_from`, `valid_to`, `change_reason` | **O que** a aula é, **a partir de quando**. Exclusion constraint: um slot não tem duas versões no mesmo dia |
| `slot_version_teachers` | `slot_version_id`, `teacher_id`, `role` (TITULAR/AUXILIAR), `duration_min?` | Suporta aula com 2 professores (coach + estagiário). No caso comum, uma linha |
...
| Tabela | Campos-chave | Notas |
|---|---|---|
| `payroll_periods` | `year`, `month`, `status`, `generated_at`, `closed_at`, `closed_by`, `needs_review`, `input_hash` | Unique `(year, month)`. Máquina de estados em [03](03-regras-de-negocio.md#6-fechamento-mensal) |
...
## 8. Financeiro (separado do operacional)

| Tabela | Campos-chave | Notas |
|---|---|---|
| `rate_tables` | `name` ("Tabela Níveis 2026") | |
| `rate_table_values` | `rate_table_id`, `level_id`, `modality_id?`, `class_type_id?`, `hourly_cents`, `valid_from`, `valid_to` | `modality_id` null = vale para todas |
| `teacher_rates` | `teacher_id`, `modality_id?`, `class_type_id?`, `hourly_cents`, `valid_from`, `valid_to` | Override individual. Precedência em [03](03-regras-de-negocio.md#7-motor-financeiro) |
| `pay_rules` | `name`, `kind` (ADICIONAL_PCT/ADICIONAL_FIXO_HORA/GRATIFICACAO_FIXA/DESCONTO/DSR), `value`, `conditions jsonb` (weekday, is_holiday, modality_ids, contract_type_ids, teacher_ids, position_ids), `priority`, `valid_from`, `valid_to`, `active` | Motor de regras declarativo, sem código novo por regra |
...
| `import_batches` / `import_rows` | arquivo, mapeamento de colunas, status por linha, erros |
...
| Índice | Atende |
|---|---|
| `class_occurrences (period_id, date)` | Calendário do mês, geração |
